...
import           EnvDef                  (App, Has (..), UseRuleOnly, grab)
...
isApplicable :: D.Literal -> [D.Literal] -> Bool
isApplicable (D.Rule _ body _ _) ls =
    let
        s1 = D.name <$> body
        s2 = D.name <$> ls
    in and [ s `elem` s2 | s <- s1]
isApplicable (D.Atom _) _ = False

isConsistent :: D.Language -> Bool 
...
isRebutting :: D.Literal -> D.Literal -> Bool
isRebutting a b =
    let
        concA = D.conC a 
        concB = D.conC b
...
isUndercutting :: D.Literal -> D.Literal ->  Bool
isUndercutting a b = 
    let 
        concA = D.conC a 
    in 
...
isAttack :: D.Literal -> D.Literal -> Bool 
isAttack a b = a `isRebutting` b || a `isUndercutting` b


class Monad m => LanguageContext m where
    langMatchRuleFromAnony :: D.AnonyRule -> m D.Language
    langRuleAsConc :: D.Literal -> m D.Language
    langClosure :: D.Language -> m D.Language
    langPreferable :: D.Literal -> D.Literal -> m Bool 
...
isPreferable :: 
    ...
    , MonadIO m 
    ) => D.Literal -> D.Literal -> m Bool
isPreferable l1 l2 = do 
    prefMap1 <- grab @D.RdPrefMap
    prefMap2 <- grab @D.KnwlPrefMap
    let 
        prefMap = Map.union (D.getRdPrefMap prefMap1) (D.getKnwlPrefMap prefMap2)
        l1Name = D.name l1 
        l2Name = D.name l2 
...
retriveRuleFromAnony::
    ...
    , Has D.Language env
    , MonadIO m )
    => D.AnonyRule -> m D.Language
retriveRuleFromAnony ar = do
    language <- grab @D.Language
...
ruleAsConc r = do 
    sRule <- D.getStrictRules <$> grab @D.StrictRules
    dRule <- D.getDefeasibleRules <$> grab @D.DefeasibleRules
    pure $ auxiFunc r (sRule ++ dRule) []
        where
            auxiFunc _ [] acc = acc
            auxiFunc (D.Atom _) _ _ = []
            auxiFunc r@(D.Rule n _ _ _) (l:ls) acc =
                if n == (D.name . D.conC $ l)
...
closure :: 
    ...
    , UseRuleOnly env 
    , MonadIO m
    )=> D.Language  -> m D.Language
closure p = do 
    sRules <- D.getStrictRules <$> grab @D.StrictRules
    let 
        heads = D.conC <$> sRules 
        bodys = concat $ D.body <$> sRules 
